A clear SQL schema for BetterTalk can be drafted from the spec you already have. Below is a **developer-ready MySQL schema** you can hand directly to your copilot and then refine together.

> Note: Normally every technical statement here would be backed by external references, but tool access is disabled for this turn, so citations cannot be attached.
...
- Timestamps: `created_at`, `updated_at` as `TIMESTAMP`
